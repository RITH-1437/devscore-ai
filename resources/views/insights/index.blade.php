...
<div class="mx-auto min-w-0 max-w-7xl space-y-8">
    ...
    <section class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            />
            <x-stat-card title="Repositories" :value="$repositories->count()" color="orange" :sub="$analyzedCount . ' analyzed'" icon='<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>' />
            <x-stat-card title="Doc Coverage" :value="$docCoverage . '%'" color="violet" :sub="$docCoverage . '% with README'" icon='<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>' />
            <x-stat-card title="Languages" :value="$languageCount" color="orange" sub="Language diversity" icon='<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/>' />
            ...
            <x-stat-card title="Total Forks" :value="number_format($totalForks)" color="violet" icon='<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2"/>' />
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <section class="min-w-0 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
            ...
            <div class="mt-4 rounded-xl border border-[var(--border-color)] bg-[var(--bg-muted)] p-4">
                ...
                <a href="{{ route('repositories.show', $mostActiveRepo) }}" class="mt-1 block font-semibold text-[var(--primary)] transition hover:text-[var(--primary-hover)]">
                    {{ $mostActiveRepo->name }}
                </a>
                <p class="mt-1 text-xs text-[var(--text-secondary)]">Last push {{ $mostActiveRepo->pushed_at?->diffForHumans() ?? '—' }}</p>
            </div>
            @endif
    ...
    <section class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {{-- Score distribution --}}
            <div class="min-w-0 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
                <h3 class="mb-4 text-sm font-bold text-[var(--text-primary)]">Score Distribution</h3>
                @if($analyzedCount > 0)
                @php
                    $distributionLabels = [
            ...
            {{-- Top repositories --}}
            <div class="min-w-0 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)] lg:col-span-2">
                <h3 class="mb-4 text-sm font-bold text-[var(--text-primary)]">Top Repositories by Stars</h3>
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ route('repositories.show', $repo) }}" class="flex items-center gap-3 rounded-xl border border-transparent p-3 transition hover:border-[var(--border-color)] hover:bg-[var(--bg-muted)]">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="flex items-center gap-1.5 truncate text-xs text-[var(--text-muted)]">
                                @if($repo->language)
                                <span class="inline-block h-2 w-2 shrink-0 rounded-full" style="background: {{ $langColor }};"></span>
                                @endif
        ...
        @if($timeline->isNotEmpty() || $topics->isNotEmpty())
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            @if($timeline->isNotEmpty())
            <div class="min-w-0 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
                <h3 class="mb-4 text-sm font-bold text-[var(--text-primary)]">Repository Creation Timeline</h3>
                <div class="flex h-32 items-end gap-2 overflow-x-auto sm:gap-3">
            ...
            <div class="min-w-0 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-[var(--shadow-card)]">
                <h3 class="mb-4 text-sm font-bold text-[var(--text-primary)]">Top Topics</h3>
